...
bool ServoEsp::init()
{
    int pin = PWM::cfg_.pin;
    auto& mc = mc_;
    log_i("Init servo esp on pin "+ to_string(pin) +"...");
    ...
    */
    ...
    int us0 = ServoPWM::cfg_.dgr2us(ServoPWM::cfg_.dgr_init);
    ESP_ERROR_CHECK(mcpwm_comparator_set_compare_value(comparator, us0));
...
bool ServoEsp::set_degree(float ang) 
{
    ...
    int us = ServoPWM::cfg_.dgr2us(ang);
    string s = "us= " + to_string(us);
    log_i(s);
